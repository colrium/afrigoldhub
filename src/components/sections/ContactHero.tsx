...
export default function ContactHero() {
	...
	return (
		<section className="relative overflow-hidden bg-surface-900 pt-24 pb-20 md:pt-32 md:pb-28">
			<div
				...
				<div className="rounded-lg border border-onSurface-100/20 bg-surface/85 p-8 shimmer-y shimmer-subtle">
					...
					<div className="grid gap-5">
						{highlightedReasons.map((reason) => (
							<div key={reason.value} className="border-t border-surface-600 pt-5">
								<div className="text-sm text-onSurface-100">{reason.label}</div>
								<div className="text-xs text-onSurface-200 mt-1">
									{offices[0]?.hours}
								</div>
							</div>
						))}
